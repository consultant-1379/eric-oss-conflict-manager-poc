...
ARG USER_NAME="eric-oss-conflict-manager-poc"
RUN echo "$USER_ID:x:$USER_ID:0:An Identity for $USER_NAME:/nonexistent:/bin/false" >>/etc/passwd

ARG JAR_FILE
ADD ./eric-oss-conflict-manager-poc-app/target/${JAR_FILE} eric-oss-conflict-manager-poc-app.jar
COPY ./eric-oss-conflict-manager-poc-app/src/main/resources/jmx/* /jmx/
...
CMD ["/bin/sh", "-c", "java ${JAVA_OPTS} -Dcom.sun.management.jmxremote=true \
    ...
    -jar eric-oss-conflict-manager-poc-app.jar"]
...
LABEL \
    org.opencontainers.image.title=eric-oss-conflict-manager-poc-jsb \
