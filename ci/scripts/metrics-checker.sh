...
checkCoreApplicationJAVA(){
    ...
    find ./eric-oss-conflict-manager-poc-app/src/main/java -regex '.*Application.java' -exec grep -o 'public RestTemplate restTemplate(final RestTemplateBuilder restTemplateBuilder)' {} \; > .bob/var.scrapedLine || true;
    find ./eric-oss-conflict-manager-poc-app/src/main/java -regex '.*Application.java' -exec grep -o 'return restTemplateBuilder.build()' {} \; >> .bob/var.scrapedLine || true;
